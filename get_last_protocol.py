...
def get_last_protocol(self,id):
    ...
    try:
        ...
        headphone_model = data_row["Sluchawki_model"]
        ...
        lastest_month = str(lastest_month)
        ...
        for i in range(1,length_s):
            ...
            if doc_tmp.paragraphs[i].text == "Paste_nr":
                ...
                center_row = doc_new.add_paragraph("pracownikiem firmy - „zdający” – %s %s"%(fname,lname),style = "Paragraph")
                center_row.alignment = 1
                ...
                if headphones == 'tak':
                    add_table_row(str(lp),f'Słuchawki przewodowe {headphone_model}',"-")
                    lp += 1
                ...
                if additional_devaice != 'nie' and additional_devaice != '':
                    if dev_imei != '' and dev_imei != '-' and dev_sn != '' and dev_sn != '-':
                        add_table_row(str(lp),"%s %s %s"%(dev_type,dev_vendor,dev_model ),"S/N %s\nIMEI %s"%(dev_sn,dev_imei))
                        lp += 1
                    elif dev_sn != '' and dev_sn != '-' :
                        add_table_row(str(lp),"%s %s %s"%(dev_type,dev_vendor,dev_model ),"S/N %s"%(dev_sn))
                        lp += 1
                    elif dev_imei == '' or dev_imei == '-' and dev_sn == '' or dev_sn == '-':
                        add_table_row(str(lp),"%s %s %s"%(dev_type,dev_vendor,dev_model ),"-")
                        lp += 1

                if additional_devaice_2 != 'nie' and additional_devaice_2 != '':
                    if dev_imei_2 != '' and dev_imei_2 != '-' and dev_sn_2 != '' and dev_sn_2 != '-':
                        add_table_row(str(lp),"%s %s %s"%(dev_type_2,dev_vendor_2,dev_model_2 ),"S/N %s\nIMEI %s"%(dev_sn_2,dev_imei_2))
                        lp += 1
                    elif dev_sn_2 != '' and dev_sn_2 != '-' :
                        add_table_row(str(lp),"%s %s %s"%(dev_type_2,dev_vendor_2,dev_model_2 ),"S/N %s"%(dev_sn_2))
                        lp += 1
                    elif dev_imei_2 == '' or dev_imei_2 == '-' and dev_sn_2 == '' or dev_sn_2 == '-':
                        add_table_row(str(lp),"%s %s %s"%(dev_type_2,dev_vendor_2,dev_model_2 ),"-")
                        lp += 1
